...
for file in ./checkpoints/*DIOR-base*.pth; do
    if [ -f "$file" ]; then
      file=$(basename $file)
      python -u dior_test.py --phase 'base' --test_mode 'test' --weight_file $file >> ./log/$filename
    fi
done
...
for file in ./checkpoints/*DIOR-inc*.pth; do
    if [ -f "$file" ]; then
      file=$(basename $file)
      python -u dior_test.py --phase 'inc' --test_mode 'test' --weight_file $file >> ./log/$filename
    fi
done
